...
from dataset.video_utils.utils import read_frames_decord, read_frames_gif
...
# modified evaluation
#
# changed for loop to suite mappable dataset instead of iterable dataset
# took out metric refrences to be done in sepearate function after completion of evaluation
# created an inner list for results that gets appended to outer list
@torch.no_grad()
    ...
    for video, video_ids, gold_caption in data_loader:
        ...
        for video_id, topk_id, topk_prob, gold_caption_list in zip(video_ids, topk_ids, topk_probs, gold_caption):
            ...
            if config["prompt"] != "":
                ...
                gold_caption_list = ''.join(gold_caption_list)
        ...
        # result_metrics = cal_metric(result)
        # metrics.append(result_metrics.item())
    return caption_results, metrics


# new test function
#
# tests one video, given video name
@torch.no_grad()
def test(model, tokenizer, device, config):
    model.to(device=device, non_blocking=True)
    model.eval()
    
    video_path = f'{config["video_root"]}/{args.test_file}.mp4'
    if not os.path.exists(video_path):
        print(f'{video_path} does not exist')

    with torch.no_grad():
        video_array = read_frames_decord(video_path, num_frames=16, sample='middle')
        ...
        video = video_array.to(device=device)
        topk_ids, topk_prob = model(video, train=False)
...
# modified calc_metric
#
# added condition to handle list input, rather than file input
def cal_metric(result_file):
    if result_file is not list:
...
# modified main
#
# removed all refrences to args.distributed
# adjusted create_dataset, create_dataloader calls to fit updated versions
# only tests
def main(args, config):
    ...
    datasets, dataset2 = create_dataset('video_caption', config)    # separated datasets
    samplers = [None, None]

    train_loader = None
    val_loader = create_loader(dataset2, None, config['batch_size_test'], 0, False, test_collect_fn)
    # Dataset1:{datasets}\n
    # print(f'Dataset1:{len(datasets)}')
    print(f'Dataset2:{len(dataset2)}')
    print(f'Loader:{val_loader}')
    ...
    if args.test_file is not None:
        test_caption_result = test(model, tokenizer, device, config)
        test_caption_file = save_result(test_caption_result, args.result_dir, f'test_{args.test_file}')

    else:
        caption_result, metrics = evaluation(model, val_loader, tokenizer, device, config)
        result_file = save_result(caption_result, args.result_dir, 'caption_jasminesvideos')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs_video/VideoCaption_msrvtt_large.yaml')
    # parser.add_argument('--config', default='./configs/VQA.yaml')
    parser.add_argument('--checkpoint', default='mPLUG2_MSRVTT_Caption.pth')
    ...
    args.result_dir = f'{args.output_dir}/result'
